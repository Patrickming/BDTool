...
│   │   │   ├── Login.tsx         # 登录页
│   │   │   ├── Register.tsx      # 注册页
│   │   │   ├── Profile.tsx       # 个人资料页（头像上传、信息编辑、密码修改）
...
│   │   │   │   ├── routes/       # 路由
│   │   │   │   └── dto/          # 数据传输对象
│   │   │   │       ├── update-profile.dto.ts  # 更新资料验证
│   │   │   │       └── change-password.dto.ts # 修改密码验证
│   │   │   ├── kol/              # KOL 管理模块
│   │   │   ├── templates/        # 模板管理模块
│   │   │   ├── analytics/        # 数据分析模块
│   │   │   ├── translation/      # 翻译服务模块
...
- ✅ **个人资料管理**
  - 头像上传（支持 PNG/JPG/GIF，最大 5MB）
  - 个人信息编辑（姓名、邮箱、公司）
  - 密码修改功能
  - 文件安全存储（uploads/avatars/）
  - 跨域资源共享（CORS）配置
...
### v1.9.0（已完成 2025-12-11）✅

- ✅ **个人资料管理系统**
  - 头像上传功能（支持 PNG/JPG/GIF，最大 5MB）
  - 个人信息编辑（姓名、邮箱、公司）
  - 密码修改功能（当前密码验证）
  - Multer 文件上传中间件集成
  - 文件存储系统（uploads/avatars/）
...
- ✅ **首页优化**
  ...
  - 用户信息实时同步更新
- ✅ **数据库迁移**
  - User 表新增 `company` 字段（公司名称）
  - User 表新增 `avatar` 字段（头像文件名）
- ✅ **UI/UX 优化**
  - Profile 页面设计（Solana 风格）
  - 头像上传交互优化（点击上传 + 悬浮效果）
  - 右上角用户头像显示
  - 个人资料编辑弹窗
  - 密码修改弹窗
...
- **v1.9.0 (2025-12-11)**: 个人资料管理、头像上传、用户状态持久化、首页优化
