...
**后端任务：**
- [ ] 创建 DTO 层
  - create-kol.dto.ts - 创建 KOL 验证
  - update-kol.dto.ts - 更新 KOL 验证
  - kol-query.dto.ts - 查询参数验证
- [ ] 创建 Service 层（kol.service.ts）
  - createKOL() - 创建单个 KOL
  ...
  - updateKOL() - 更新信息
  - deleteKOL() - 删除 KOL
  - parseTwitterInput() - 解析 Twitter 输入
- [ ] 创建 Controller 层（kol.controller.ts）
  - 所有 HTTP 端点处理
  - 错误处理和响应标准化
- [ ] 创建 Routes 层（kol.routes.ts）
