...
#### 3.2 前端开发 ⏳ 待开始
**前端任务：**
- [ ] 创建类型定义（types/template.ts）（30 分钟）
- [ ] 创建 API 服务层（services/template.service.ts）（30 分钟）
- [ ] 创建状态管理（store/template.store.ts）（45 分钟）
- [ ] 创建复用组件（2 小时）
  - TemplateEditor.tsx - 模板编辑器（含变量插入）
  - TemplatePreview.tsx - 实时预览组件
  - TemplateCategoryBadge.tsx - 分类标签
  - VariableHelper.tsx - 变量帮助面板
- [ ] 创建页面组件（1.5 小时）
  - TemplateList.tsx - 列表页面
