...
import { Card, Typography, Steps, Button, message, Modal, Statistic, Tag, Alert, Collapse, Space, Row, Col } from 'antd';
import {
  RocketOutlined,
  DownloadOutlined,
  SafetyOutlined,
...
export default function Extension() {
  ...
  const loadExtensionToken = async () => {
    try {
      const response = await fetch('http://localhost:3000/api/v1/extension/token', {
        headers: {
  ...
  const handleGenerateToken = async () => {
    ...
    try {
      const response = await fetch('http://localhost:3000/api/v1/extension/token/generate', {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('auth_token')}`,
        },
  ...
  return (
    <div>
          ...
          <Col xs={24} lg={12}>
            {extensionToken ? (
              ...
              >
                ...
                <Button type="primary" icon={<ApiOutlined />} onClick={handleGenerateToken} loading={isGenerating}>
                  生成 Token
                </Button>
              </Card>
            )}
            ...
            >
              <div style={{ textAlign: 'center' }}>
                ...
                <div>
                  ...
                  <Title level={2} style={{ margin: 0, color: tokenExpiry ? '#FF8C00' : 'rgba(255, 255, 255, 0.3)' }}>
        ...
        <Row gutter={[24, 24]}>
          <Col xs={24} md={8}>
            <div style={{ textAlign: 'center', padding: 24 }}>
              <TwitterOutlined style={{ fontSize: 48, color: '#9945FF', marginBottom: 16 }} />
          ...
          <Col xs={24} md={8}>
            <div style={{ textAlign: 'center', padding: 24 }}>
              <SafetyOutlined style={{ fontSize: 48, color: '#14F195', marginBottom: 16 }} />
              <Title level={4} style={{ color: '#fff', marginBottom: 12 }}>
                本地编辑
              </Title>
              <Text style={{ color: 'rgba(255, 255, 255, 0.6)' }}>
          ...
          <Col xs={24} md={8}>
            <div style={{ textAlign: 'center', padding: 24 }}>
              <CheckCircleOutlined style={{ fontSize: 48, color: '#00D4AA', marginBottom: 16 }} />
              <Title level={4} style={{ color: '#fff', marginBottom: 12 }}>
                批量上传
              </Title>
              <Text style={{ color: 'rgba(255, 255, 255, 0.6)' }}>
      ...
      >
        ...
        <Steps
          ...
          items={[
            ...
            {
              title: <span style={{ color: '#fff', fontSize: 16 }}>开启开发者模式</span>,
              description: (
                <Paragraph style={{ color: 'rgba(255, 255, 255, 0.6)', marginTop: 12 }}>
            ...
            {
              title: <span style={{ color: '#fff', fontSize: 16 }}>加载插件</span>,
              description: (
                <Paragraph style={{ color: 'rgba(255, 255, 255, 0.6)', marginTop: 12 }}>
                  1. 点击「加载已解压的扩展程序」
                  <br />
            ...
            {
              title: <span style={{ color: '#fff', fontSize: 16 }}>配置 Token</span>,
              description: (
                <Paragraph style={{ color: 'rgba(255, 255, 255, 0.6)', marginTop: 12 }}>
                  1. 从本页面复制 Token
                  <br />
            ...
            {
              title: <span style={{ color: '#fff', fontSize: 16 }}>开始使用</span>,
              description: (
                <Paragraph style={{ color: 'rgba(255, 255, 255, 0.6)', marginTop: 12 }}>
                  访问 Twitter 用户主页（例如 x.com/elonmusk），点击插件图标即可开始捕获 KOL 数据
                </Paragraph>
              ),
            },
          ]}
      ...
      >
        ...
        >
          <Panel
            header={<Text style={{ color: '#fff', fontSize: 16 }}>如何捕获 KOL？</Text>}
            key="1"
          ...
          <Panel
            header={<Text style={{ color: '#fff', fontSize: 16 }}>如何填写额外信息？</Text>}
            key="2"
            style={{ borderBottom: '1px solid rgba(255, 255, 255, 0.1)' }}
          >
            <Paragraph style={{ color: 'rgba(255, 255, 255, 0.6)' }}>
          ...
          <Panel
            header={<Text style={{ color: '#fff', fontSize: 16 }}>如何上传到系统？</Text>}
            key="3"
            style={{ borderBottom: '1px solid rgba(255, 255, 255, 0.1)' }}
          >
            <Paragraph style={{ color: 'rgba(255, 255, 255, 0.6)' }}>
          ...
          <Panel
            header={<Text style={{ color: '#fff', fontSize: 16 }}>Token 过期了怎么办？</Text>}
            key="4"
          >
            <Paragraph style={{ color: 'rgba(255, 255, 255, 0.6)' }}>
